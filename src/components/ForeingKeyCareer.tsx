...
const ForeignKeyCareer: React.FC<ForeignKeySelectProps> = ({ onChange }) => {
  ...
  const fetchForeignKeyData = async () => {
    try {
      const response = await axios.get('http://localhost:8081/carreras'); // Reemplaza '/api/foreign-keys' con la ruta real a tu endpoint del backend
